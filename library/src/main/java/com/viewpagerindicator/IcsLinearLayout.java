...
class IcsLinearLayout extends LinearLayout {
    ...
    public IcsLinearLayout(Context context, int themeAttr) {
        ...
        //themeAttr:自定义一个可以在Theme中配置的样式的关键
        //set>defStyleAttr(主题可配置样式)>defStyleRes(默认样式)>NULL(主题中直接指定)
        TypedArray a = context.obtainStyledAttributes(null, LL, themeAttr, 0);

        if (a.getDrawable(IcsLinearLayout.LL_DIVIDER) ==null){
        Log.e("getDrawable 没有初始化","getDrawable getDrawable getDrawable getDrawable getDrawable 没有初始化");}
    ...
    public void setDividerDrawable(Drawable divider) {
        ...
        SetBitmap(drawableToBitmap(mDivider));
        requestLayout();
    }
